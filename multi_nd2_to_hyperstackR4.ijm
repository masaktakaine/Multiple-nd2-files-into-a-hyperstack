...
//		newImage("HyperStack", "&typeh &typed", imwidth, imheight, nchannels, imagefilelist.length, 1); // With Imagej1.43 and later adding "&" can be used instead of string concatenation.
				}
...
run("Set Label...", "label=&currch_label"); // Rename the current slice. With Imagej1.43 and later adding "&" can be used instead of string concatenation.
}
selectImage(currfileid);
close();
